...
function stringLib:substr(str,s,e)
    if not str then return "" end;
    local realS=s;
    local realE=e;
    local charStartIndex=1;
    for inx=1,e do
        if inx==s then
            realS=charStartIndex;
        end
       
        if string.byte(str,charStartIndex)>=128 then
            charStartIndex=charStartIndex+3;
        else
            charStartIndex=charStartIndex+1;
        end
    end
    realE=charStartIndex-1;
--    trace(realS,realE)
    return string.sub(str,realS,realE)
end
...
function stringLib.comparei(str1, str2)
	return stringLib.compare(string.lower(str1), string.lower(str2))
end
...
function stringLib.equali(str1, str2)
	return stringLib.equal(string.lower(str1), string.lower(str1))
end
...
function stringLib.insertRep(str, chars, rep, index)
	...
	return stringLib.insert(str, rep, index)
end
...
function stringLib.sortAscending(str)
	local chars = stringLib.toCharArray(str)
...
function stringLib.highest(str)
	local s = stringLib.sortDescending(str)
	return s[1]
end
...
function stringLib.lowest(str)
	local s = stringLib.sortAscending(str)
	return s[1]
end
...
function stringLib.wordPercents(str)
	local t = stringLib.wordTotals(str)
...
function stringLib.wordPercent(str, word)
	local t = stringLib.wordPercents(str)
...
function stringLib.charPercents(str)
	local t = stringLib.charTotals(str)
	local count = string.len(str)
...
function stringLib.charPercent(str, char)
	local t = stringLib.charPercents(str)
...
function stringLib.spacePercent(str)
	local count = stringLib.spaceCount(str)
...
function stringLib.upperPercent(str)
	local count = stringLib.upperCount(str)
	return (count / string.len(str)) * 100.0
end
...
function stringLib.lowerPercent(str)
	local count = stringLib.lowerCount(str)
	return (count / string.len(str)) * 100.0
end
...
function stringLib.digitPercents(str)
	local t = stringLib.digitTotals(str)
	local count = string.len(str)
...
function stringLib.digitPercent(str, digit)
	local t = stringLib.digitPercents(str)
...
function stringLib.puncPercents(str)
	local t = stringLib.puncTotals(str)
	local count = string.len(str)
...
function stringLib.puncPercent(str, punc)
	local t = stringLib.puncPercents(str)
...
function dectobin(dec, limit)
	...
	if (padding ~= limit) then
		bin = stringLib.insertRep(bin, '0', padding, 0)
	end
...
function stringLib.uuencode(str)
	...
	if (padding ~= 3) then
		str = stringLib.insertRep(str, string.char(1), padding, string.len(str))
	end
...
function stringLib.uudecode(str)	
	...
	return stringLib.trim(uudec, string.char(1))
end
