...
function BulletBase:checkHit()
    ...
    local bulletSize = self.sprite_:getContentSize()
    local w ,h=bulletSize.width*0.5,bulletSize.height*0.5 

    -- local targetX = target.x_ + target.radiusOffsetX_
    -- local targetY = target.y_ + target.radiusOffsetY_
    local targetX = target.x_
    local targetY = target.y_
    local dist = math2d.dist(x, y, targetX, targetY);
    -- local bulletRadius =  math.sqrt(w * w + h * h);
    local bulletRadius =  0;
    if dist <= target.radius_ + bulletRadius then
        self:hit()
        return true
    else
        print("x y targetX, targetY",x,y,targetX,targetY)
        print("dist " .. dist);
        print("target.radius_ " .. target.radius_);
        self:miss()
        return false
    end
