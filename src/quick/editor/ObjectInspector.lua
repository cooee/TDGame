...
local BehaviorFactory = require("game.map.behaviors.BehaviorFactory")
...
function ObjectInspector:setPosition()
    ...
    print("position_ " .. self.position_)
    ...
    elseif self.position_ == ObjectInspector.POSITION_LEFT_BOTTOM then
        ...
        local y = display.bottom + height + offset
        if self.scale_ == 1 then
            y = y + EditorConstants.MAP_TOOLBAR_HEIGHT * self.toolbarLines_
        end
        self.sprite_:align(display.LEFT_TOP, display.right - width - offset, y)
    end
