...
function MapRuntime:onTouch(event, x, y)
    ...
    if selectedTowerId then
        -- 对选中的塔做操作
        print("-- 对选中的塔做操作")
        local tab = self.map_:getAllObjects();
        local object = tab[selectedTowerId];
        if object then
            -- dump(object);
            -- if object:getView() then
...
local function checkStiaticObjectCollisionEnabled(obj)
    return ((obj.classIndex_ == CLASS_INDEX_STATIC) or (obj.classIndex_ == CLASS_INDEX_MOVE))
