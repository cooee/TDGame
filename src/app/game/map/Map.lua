...
local MapCamera     = require("app.game.map.MapCamera")
local ObjectFactory = require("app.game.map.ObjectFactory")
local Path          = require("app.game.map.Path")
...
function Map:ctor(id, debug)
    ...
    self.ready_            = false
    self.mapModuleName_    = string.format("app.game.maps.Map%sData", id)
    self.eventModuleName_  = string.format("app.game.maps.Map%sEvents", id)
...
function Map:setObjectDebugViewEnabled(isDebugViewEnabled,object)
    self.debugViewEnabled_ = isDebugViewEnabled
    for id, object in pairs(self.objects_) do
        object:setDebugViewEnabled(false)
    end
    object:setDebugViewEnabled(isDebugViewEnabled);
    if self:getDebugLayer() then
        self:getDebugLayer():setVisible(isDebugViewEnabled)
...
function Map:checkTowerPos(x,y)
   -- CCSprite* tileAt(CCPoint tileCoordinate);
    local TileCoor = self:getTileCoor(x,y);
    local sprite = self.meta_:tileAt(TileCoor);
    -- local pos= self.meta_:positionAt(ccp(2,4));
    -- local x2,y2 = sprite:getPosition();
    -- print("x=%d  y=%d", pos.x,pos.y)
    -- print("x2=%d  y2=%d", x2,y2)
    if sprite and sprite:isVisible() then
        return true;
    end
    return false;
end

function Map:getTileCoor(x,y)
   local tileX = x / 80;
   local tileY = y / 80;
...
function Map:getTowerPos(x,y)
    local TileCoor = self:getTileCoor(x,y);
    local pos = self.meta_:positionAt(TileCoor);
    pos.x = pos.x + 40;
    pos.y = pos.y + 40;
    return pos;
end
